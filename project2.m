...
h1 = quiver(x,y*100,dV_dt,dw_dt*100);
set(h1,'LineWidth',2,'AutoScale','on', 'AutoScaleFactor', 1)
...
ylabel('Firing Rate (in hz or 1/s)');
xlabel('Iext (in uA)');
title('Firing Rate vs External Current (Graph diff from writeup as this graph uses lesser number of points to reduce number of plots generated. Writeup uses more points)');
...
function hodgkin_huxley
    ...
    %% Anode Break excitation effect (Question 17) 
    figure;
    tSpan1 = [0 20];
    ...
    grid on;
     ...
     %% Phase plane of the reduced HH model (Question 18) 
    fprintf('\n------------------------- Part 18 ------------------------------ \n');
    Vr = -60;
